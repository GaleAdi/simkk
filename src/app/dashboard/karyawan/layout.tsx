...
export default async function KaryawanDashboardLayout({
  children,
}: {
  ...
  if (session.user.role !== "KARYAWAN") redirect("/dashboard/hrd");

  const karyawan = await prisma.karyawan.findUnique({
    where: { userId: session.user.id },
  });
  const userName = karyawan?.nama || session.user.username;
  const now = new Date();

  return (
    <div className="flex min-h-screen bg-gray-50">
      <Sidebar userName={userName} userRole="KARYAWAN" />
